...
// import { fetchImg } from '../store/dataSlice';
...
function Button({data}) {
  ...
  // const { items, status, retries } = useSelector((state) => state.pokeImg);
	// const getImg = (url) => {
	// 	console.log(items)
  //   if (status !== 'succeeded' && status !== 'loading' && retries < 3 && items === "") {
  //     return dispatch(fetchImg(url));
  //   } else {
	// 		return "";
	// 	}
	// }
			...
			<div className="cardList">
				{
					data.map((items, index) => {
						...
						if (fullRand[6] < 10000){
							...
							// const url = getImg("https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/"+fullRand[6]+".png");
